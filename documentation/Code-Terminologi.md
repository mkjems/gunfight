...
| Duel         | One showdown cycle inside a match. Code uses `duelNumber` for the duel count and next scenario selection.                                           | `duelNumber` in `gfmodel.ts` and `PublicGameModel`    |
| Phase        | Server-owned lifecycle value: `waiting`, `readying`, `readyCountdown`, `duelIntro`, `playing`, `hitPause`, `gameOver`, `abandoned`, or `closed`.    | `GAME_PHASE` in `shared/contracts.ts`                 |
| Duel state   | Client presentation state: `waiting`, `ritual`, `playing`, `hitPause`, `duelOver`, or `gameOver`. It follows server phase but is not authoritative. | `client/src/state/clientScreens.ts`                   |
...
| Term          | Meaning                                                                                                                                                | Main code                                                      |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| Public model  | Server-published state needed by clients: clients, names, slots, phase, timing, scenario, duel number, match state, scores, and version.               | `PublicGameModel` in `shared/contracts.ts`                     |
...
| Game model    | Server module that owns phase, timing, ready flags, scenario, duel number, match state, scores, and version inside one session.                        | `server/gameModules/gfmodel.ts`                                |
...
| Money bag       | Passive timed scenario object for now. It appears after legacy field `gameRoundSeconds` and may become part of future progression rules. | `MoneyBagInstance`, `scenarioRenderer.ts`                    |
...
| Hit             | Client-detected bullet/player contact. The winning client reports `duelResult`; the server accepts only current, valid results.          | `client/src/flows/clientPlayerHitFlow.ts`, `gfmodel.ts`      |
...
| Shooting straightness    | Player-held value used for newly fired local bullets. It is currently derived from duel number and frozen into each bullet when fired.        | `Controllable.shootingStraightness`, `Players.ensure`          |
| Duel bullet straightness | Current helper that derives shooting straightness from `duelNumber` and bullet config.                                                        | `getDuelBulletStraightness` in `client/src/platform/config.ts` |
| Straightness meter       | Visual indicator under a gameplay player showing the straightness value the next shot will use. In code, prefer `straightnessMeter`.          | `Config.straightnessMeter`, `Controllable`                     |
...
| Resting                  | A low-straightness bullet has stopped and stays as scenery for the current duel. Resting bullets do not block another shot.                   | `Bullet.isResting`                                             |
...
| HUD            | DOM overlay for score, names, ammo, duel messages, lobby text, high scores, name editor, and touch controls.                  | `client/src/ui/`, `ClientHudFlow`      |
...
| `duelResult`     | Winning client reports a hit result. Server validates and updates score/phase.                  | Client to server |
...
- `Player`: Ambiguous because the server has connected clients and the canvas
  ...
  matters.
- `gameRoundSeconds`: Legacy scenario field for timed money bags. Leave it
  unchanged until money bag gameplay settles.
